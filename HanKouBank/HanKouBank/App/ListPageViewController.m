...
- (void)viewDidLoad
{
    ...
    for (int i = 0; i<[self.arr count]; i++) {
        ...
        //listpage中保存数据模型数据
        [self.strArray addObject:((MainLevel *)[self.arr objectAtIndex:i]).text];
        [self.idArray addObject:((MainLevel *)[self.arr objectAtIndex:i]).Id];
        [self.sortArray addObject:((MainLevel *)[self.arr objectAtIndex:i]).sortFlag];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //topic
    [cell.ListCell_title setText:((ListLevel *)[self.listArray objectAtIndex:[indexPath row]]).topic];
    
    //时间格式处理
    NSString *date =[[NSString alloc]init];
   date = [date DateToChanged:((ListLevel *)[self.listArray objectAtIndex:[indexPath row]]).createDate];
    [cell.ListCell_date setText:date];
    //summery
    [cell.ListCell_titleless setText:((ListLevel *)[self.listArray objectAtIndex:[indexPath row]]).summary];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    contentPageView.Content_docIdStr = [NSString stringWithFormat:@"%@",((ListLevel *)[self.listArray objectAtIndex:[indexPath row]]).docId];
    ...
    if (request.tag == 210) {
        ...
        for (NSDictionary *dic  in array) {
            for (NSDictionary *d in [dic objectForKey:@"content"]) {
                ListLevel *list_level = [[ListLevel alloc]initLevelWithDic:d];

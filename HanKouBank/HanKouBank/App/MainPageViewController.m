    ...
    if (request.tag == 200) {
      //  NSLog(@"request:%@",request.responseString);
        ...
        for (NSDictionary *dic in arr)
        {
            ...
            MainLevel *level = [[MainLevel alloc]initLevelWithDic:dic];
            [self.mArray addObject:level];
        }
            [self.listDict setObject:self.tableTitleArray forKey:@"text"];
            [self.listDict setObject:self.channelIconArray forKey:@"channelIconArray"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.MainCell_Title setText:[NSString stringWithFormat:@"%@",((MainLevel *)[[self.listDict objectForKey:@"text"] objectAtIndex: [indexPath row]]).text]];
    ...
    [cell.MainCell_ImageView setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@",[[self.listDict objectForKey:@"channelIconArray"] objectAtIndex:[indexPath row]]]]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [listPageView.navigationItem setTitle:[NSString stringWithFormat:@"%@",((MainLevel *)[self.tableTitleArray objectAtIndex:[indexPath row]]).text]];
    listPageView.arr = ((MainLevel *)[self.mArray objectAtIndex:[indexPath row]]).sons;
